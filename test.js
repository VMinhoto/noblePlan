...
const loadVillageData = () => {
    const lastCheckedAt = localStorage.getItem('villageList_lastCheckedAt');
    if (!lastCheckedAt || Math.abs(new Date().getTime() - new Date(lastCheckedAt).getTime()) / 36e5 > 1) {
        console.log(lastCheckedAt, 'over 1 hour has past since last check. Refreshing village list.txt');
        twLib.get({
            ...
            success: function (villages) {
                ...
                villages.match(/[^\r\n]+/g).forEach(villageData => {
                    ...
                    villageOverviewList[coordinates] = {id: id, name: name, player_id: player_id};
                });
                localStorage.setItem('villagesList_' + game_data.world, LZString.compressToUTF16(JSON.stringify(villageOverviewList)));
                localStorage.setItem('villageList_lastCheckedAt', new Date());
                storedVillageList = villageOverviewList;
        ...
        storedVillageList = JSON.parse(LZString.decompressFromUTF16(localStorage.getItem(`villagesList_${game_data.world}`)));
    }
}
...
async function initializeScript() {
    ...
    <div style="width: 600px">
        ...
        <div style="margin-top: 5px">
            <b>Coordinates:</b><br>
            <textarea oninput="validateCoordinatesInput()" id="targetsToLookup" style=" width: 97%;" rows="5"></textarea>
            <br>
            <input id="fetchCommands" type="button" class="btn btn-default" value="Fetch Commands" disabled onclick="fetchCommands()">
        </div>
    </div>`);
}
...
function resetLocalStorage() {
    localStorage.removeItem('villagesList_' + game_data.world);
...
function fetchCommands() {
    ...
    targetsToLookup.forEach((t) => {
        const targetId = storedVillageList[t].id;
    ...
    $('#commandCenter_extract').click(() => {
        ...
        const planningInput = rows.map((r) => {
            ...
            const formattedDate = date.toISOString().slice(0, -5);
